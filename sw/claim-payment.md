# Malipo ya Madai (Claim Payment)

Rasilimali ya Malipo ya Madai inashughulikia utumaji wa malipo ya madai kwa TIRA. Tumia `tira.claimPayment` kutuma maelezo ya malipo ya madai — ikijumuisha kiasi kilicholipwa, njia ya malipo, hali ya taarifa, taarifa za kesi, na maelezo ya wadai — dhidi ya dai lilopo, na kushughulikia majibu ya callback ya TIRA.
...
| ------------------------------------------- | --------------------------------------------- | ----------------------------------------------------------------------------- | ------------------------------------------------ |
| `tira.claimPayment.submit(payload)`         | Tuma malipo ya madai kwa TIRA                 | Baada ya dai kushughulikiwa na unahitaji kurekodi malipo                      | `ClaimPaymentResponse`                           |
| `tira.claimPayment.handleCallback(input)`   | Chambua na kutoa data kutoka callback ya TIRA | TIRA inapotuma matokeo ya utumaji wako kwa callback URL yako                  | `CallbackResult<ClaimPaymentCallbackResponse>`   |
...
Inatuma malipo ya madai kwa TIRA. Hii ni operesheni ya **asynchronous** — unapata uthibitisho mara moja, na matokeo halisi yanakuja baadaye kupitia callback URL yako.

**Endpoint:** `POST /eclaim/api/claim/claim-payment/v1/request`

### Sehemu za Malipo ya Madai
...
| `claim_payment_number`       | `string`           | Ndiyo       | —            | `ClaimPaymentNumber`       | Nambari ya malipo ya madai kwa mujibu wa bima. String(50).                                                        |
...
| `payment_date`               | `string\|Date`     | Ndiyo       | —            | `PaymentDate`              | Tarehe ya malipo. Tazama [Utumaji wa Tarehe](#utumaji-wa-tarehe).                                                 |
| `paid_amount`                | `number`           | Ndiyo       | —            | `PaidAmount`               | Kiasi kilicholipwa. Numeric(36,2).                                                                                |
| `payment_mode`               | `"1"\|"2"\|"3"`    | Ndiyo       | —            | `PaymentMode`              | Njia ya malipo. Tazama [Thamani za Njia ya Malipo](#thamani-za-njia-ya-malipo).                                   |
| `parties_notified`           | `"Y"\|"N"`         | Ndiyo       | —            | `PartiesNotified`          | Kama wahusika waliarifiwa. Tazama [Thamani za Taarifa kwa Wahusika](#thamani-za-taarifa-kwa-wahusika).            |
| `net_premium_earned`         | `number`           | Ndiyo       | —            | `NetPremiumEarned`         | Premium halisi iliyopatikana. Numeric(36,2).                                                                      |
...
| `litigation_reason`          | `string`           | Ndiyo       | —            | `LitigationReason`         | Sababu ya kesi. String(1000). Inahitajika hata kama hakuna kesi — tazama [Sababu ya Kesi](#sababu-ya-kesi).      |
...
Malipo ya madai yanatumia muundo rahisi wa wadai (`SimpleClaimant`) wenye sehemu 4 tu. Hii ni tofauti na [Madai ya Awali](/sw/claim-intimation), ambayo inahitaji maelezo kamili ya wadai (jina, tarehe ya kuzaliwa, anwani, simu, n.k.).
:::
...
### Thamani za Njia ya Malipo

| Thamani | Maelezo                            |
| ------- | ---------------------------------- |
| `"1"`   | Taslimu (Cash)                     |
| `"2"`   | Hundi (Cheque)                     |
| `"3"`   | EFT (Uhamisho wa Fedha Kielektroniki) |

### Thamani za Taarifa kwa Wahusika

| Thamani | Maelezo                                          |
| ------- | ------------------------------------------------ |
| `"Y"`   | Wahusika waliarifiwa kuhusu malipo ya madai      |
| `"N"`   | Wahusika hawakuarifiwa                           |
...
- `request_id`, `insurer_company_code`, `claim_payment_number`, `claim_reference_number`, `claim_intimation_number`, `covernote_reference_number`, `litigation_reason` — zote ni string zinazohitajika
- `callback_url` — inahitajika na lazima iwe URL halali
- `payment_date` — lazima iwe string ya tarehe halali (fomati ya ISO) au kitu cha `Date`
- `paid_amount`, `net_premium_earned` — lazima ziwe nambari halali
- `payment_mode` — lazima iwe `"1"`, `"2"`, au `"3"`
- `parties_notified` — lazima iwe `"Y"` au `"N"`
...
const result = await tira.claimPayment.submit({
  ...
  litigation_reason: "Mdai amepinga dhima na kufungua kesi Mahakama Kuu",
  currency_code: "USD",
...
Unapoita `tira.claimPayment.submit()`, unapata `ClaimPaymentResponse` mara moja kutoka TIRA:
...
Katika hatua hii, `"TIRA001"` inamaanisha TIRA imepokea ombi lako na linashughulikiwa. **Haimaanishi** malipo yako ya madai yamekubaliwa. Matokeo halisi yanakuja baadaye kupitia callback URL yako.
...
Baada ya TIRA kushughulikia utumaji wako, inatuma matokeo kwa `callback_url` yako. Callback ina matokeo halisi — kama malipo yako ya madai yamekubaliwa au yamekataliwa.
...
Wakati `response_status_code` ni `"TIRA001"`, malipo ya madai yamekubaliwa.
...
Wakati `response_status_code` ni kitu kingine tofauti na `"TIRA001"`, malipo ya madai yamekataliwa. Angalia ukurasa wa [Misimbo ya Makosa](/sw/error-codes) kwa msimbo husika.
...
app.post("/tira/claim-payment-callback", async (req, res) => {
  ...
  if (result.extracted.response_status_code === "TIRA001") {
    ...
    console.error(
      `Malipo ya madai yamekataliwa: ${result.extracted.response_status_code}`,
...
1. Kinapata jina sahihi la tag ya uthibitisho (`ClaimPaymentRes` → `ClaimPaymentResAck`)
...
| Sehemu               | Aina                             | Maelezo                                                                           |
| -------------------- | -------------------------------- | --------------------------------------------------------------------------------- |
| `type`               | `"claim_payment"`                | Daima `"claim_payment"` kwa handler hii                                           |
| `extracted`          | `ClaimPaymentCallbackResponse`   | Data iliyotolewa (tazama [Jibu la Callback](#jibu-la-callback-la-submit))         |
...
| -------------------- | ------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| Ya rasilimali maalum | `tira.claimPayment.handleCallback(input)`   | Ukiwa na endpoint tofauti kwa kila aina ya rasilimali                                          |
...
Programu kamili ya Express.js inayotuma malipo ya madai, kushughulikia callback, na kuthibitisha.
...
Kwa chaguomsingi, `currency_code` ni `"TZS"` na `exchange_rate` ni `1.0`. Ikiwa malipo ya madai yanahusisha fedha za kigeni:
...
Kiasi chote cha fedha (`paid_amount`, `net_premium_earned`) kinapaswa kuwa katika fedha iliyoainishwa. Kiwango cha ubadilishaji kinafomatiwa kwa desimali 2 kwenye XML.

### Sababu ya Kesi

Sehemu ya `litigation_reason` inahitajika daima, hata wakati `claim_resulted_litigation` ni `"N"`. Hii ni kwa sababu kifurushi kinathibitisha bila masharti.

```js
// Dai lilisababisha kesi
{
  claim_resulted_litigation: "Y",
  litigation_reason: "Mdai amepinga dhima na kufungua kesi Mahakama Kuu",
}

// Hakuna kesi — bado lazima utoe sababu
{
  claim_resulted_litigation: "N",
  litigation_reason: "Hakuna kesi iliyotokea kutokana na dai hili",
}
```

Kuacha `litigation_reason` au kutuma string tupu kutatupa `TiraValidationError` bila kujali thamani ya `claim_resulted_litigation`.

### Wadai Waliorahisishwa

Tofauti na [Madai ya Awali](/sw/claim-intimation) ambayo inahitaji maelezo kamili ya wadai (jina, tarehe ya kuzaliwa, anwani, nambari ya simu, n.k.), malipo ya madai yanatumia muundo rahisi wa wadai wenye sehemu 4 tu:
...
Sehemu ya `payment_mode` inaelezea jinsi malipo ya madai yalivyotolewa kwa mdai:
...
// Uhamisho wa fedha kielektroniki
{ payment_mode: "3" }
```
...
`"TIRA001"` kwenye jibu la utumaji inamaanisha "imepokelewa", si "imekubaliwa". Idhini au kukataliwa halisi kunakuja baadaye kupitia callback. Usiwaambie watumiaji wako malipo ya madai yamekubaliwa wakati wa utumaji.
:::
