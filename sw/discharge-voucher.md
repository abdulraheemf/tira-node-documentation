# Hati ya Malipo (Discharge Voucher)

Rasilimali ya Hati ya Malipo inashughulikia utumaji wa hati za malipo kwa TIRA. Tumia `tira.dischargeVoucher` kutuma maelezo ya hati ya malipo — ikijumuisha kiasi cha toleo la madai, taarifa za marekebisho, data ya upatanisho, na maelezo ya wadai — dhidi ya tathmini ya madai iliyopo, na kushughulikia majibu ya callback ya TIRA.
...
| --------------------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------- | -------------------------------------------------- |
| `tira.dischargeVoucher.submit(payload)`       | Tuma hati ya malipo kwa TIRA                  | Baada ya tathmini ya madai kutumwa na maelezo ya hati ya malipo yapo tayari | `DischargeVoucherResponse`                         |
| `tira.dischargeVoucher.handleCallback(input)` | Chambua na kutoa data kutoka callback ya TIRA | TIRA inapotuma matokeo ya utumaji wako kwa callback URL yako                | `CallbackResult<DischargeVoucherCallbackResponse>` |
...
Inatuma hati ya malipo kwa TIRA. Hii ni operesheni ya **asynchronous** — unapata uthibitisho mara moja, na matokeo halisi yanakuja baadaye kupitia callback URL yako.

**Endpoint:** `POST /eclaim/api/claim/claim-dischargevoucher/v1/request`

### Sehemu za Hati ya Malipo
...
| `discharge_voucher_number`       | `string`           | Ndiyo       | —            | `DischargeVoucherNumber`      | Nambari ya hati ya malipo kwa mujibu wa bima. String(50).                               |
| `claim_assessment_number`        | `string`           | Ndiyo       | —            | `ClaimAssessmentNumber`       | Nambari ya tathmini ya madai ambayo hati hii inahusiana nayo. String(50).               |
...
| `discharge_voucher_date`         | `string\|Date`     | Ndiyo       | —            | `DischargeVoucherDate`        | Tarehe ya hati ya malipo. Tazama [Utumaji wa Tarehe](#utumaji-wa-tarehe).               |
...
| `claim_offer_communication_date` | `string\|Date`     | Ndiyo       | —            | `ClaimOfferCommunicationDate` | Tarehe toleo la madai lilipowasilishwa. Tazama [Utumaji wa Tarehe](#utumaji-wa-tarehe). |
| `claim_offer_amount`             | `number`           | Ndiyo       | —            | `ClaimOfferAmount`            | Kiasi kilichotolewa kwa mdai. Numeric(36,2).                                            |
| `claimant_response_date`         | `string\|Date`     | Ndiyo       | —            | `ClaimantResponseDate`        | Tarehe mdai alipojibu. Tazama [Utumaji wa Tarehe](#utumaji-wa-tarehe).                  |
| `adjustment_date`                | `string\|Date`     | Ndiyo       | —            | `AdjustmentDate`              | Tarehe ya marekebisho. Tazama [Utumaji wa Tarehe](#utumaji-wa-tarehe).                  |
| `adjustment_reason`              | `string`           | Ndiyo       | —            | `AdjustmentReason`            | Sababu ya marekebisho. String(1000).                                                    |
| `adjustment_amount`              | `number`           | Ndiyo       | —            | `AdjustmentAmount`            | Kiasi cha marekebisho. Numeric(36,2).                                                   |
| `reconciliation_date`            | `string\|Date`     | Ndiyo       | —            | `ReconciliationDate`          | Tarehe ya upatanisho. Tazama [Utumaji wa Tarehe](#utumaji-wa-tarehe).                   |
| `reconciliation_summary`         | `string`           | Ndiyo       | —            | `ReconciliationSummary`       | Muhtasari wa upatanisho. String(1000).                                                  |
| `reconciled_amount`              | `number`           | Ndiyo       | —            | `ReconciledAmount`            | Kiasi cha mwisho cha upatanisho. Numeric(36,2).                                         |
| `offer_accepted`                 | `"Y"\|"N"`         | Ndiyo       | —            | `OfferAccepted`               | Kama mdai amekubali toleo. Y=Ndiyo, N=Hapana.                                           |
...
Hati ya malipo inatumia muundo rahisi wa wadai (`SimpleClaimant`) wenye sehemu 4 tu. Hii ni tofauti na [Madai ya Awali](/sw/claim-intimation), ambayo inahitaji maelezo kamili ya wadai (jina, tarehe ya kuzaliwa, anwani, simu, n.k.).
:::
...
### Thamani za Kukubali Toleo

| Thamani | Maelezo               |
| ------- | --------------------- |
| `"Y"`   | Mdai amekubali toleo  |
| `"N"`   | Mdai hakukubali toleo |
...
- `request_id`, `insurer_company_code`, `discharge_voucher_number`, `claim_assessment_number`, `claim_reference_number`, `covernote_reference_number`, `adjustment_reason`, `reconciliation_summary` — zote ni string zinazohitajika
- `callback_url` — inahitajika na lazima iwe URL halali
- `discharge_voucher_date`, `claim_offer_communication_date`, `claimant_response_date`, `adjustment_date`, `reconciliation_date` — lazima ziwe string za tarehe halali (fomati ya ISO) au vitu vya `Date`
- `claim_offer_amount`, `adjustment_amount`, `reconciled_amount` — lazima ziwe nambari halali
- `offer_accepted` — lazima iwe `"Y"` au `"N"`
...
const result = await tira.dischargeVoucher.submit({
  ...
  adjustment_reason: "Marekebisho baada ya mapitio zaidi ya nyaraka za madai",
  adjustment_amount: 1000000.0,
  reconciliation_date: "2020-09-10T13:55:22",
  reconciliation_summary:
    "Upatanisho wa mwisho baada ya marekebisho na mazungumzo na mdai",
...
Unapoita `tira.dischargeVoucher.submit()`, unapata `DischargeVoucherResponse` mara moja kutoka TIRA:
...
Katika hatua hii, `"TIRA001"` inamaanisha TIRA imepokea ombi lako na linashughulikiwa. **Haimaanishi** hati yako ya malipo imekubaliwa. Matokeo halisi yanakuja baadaye kupitia callback URL yako.
...
Baada ya TIRA kushughulikia utumaji wako, inatuma matokeo kwa `callback_url` yako. Callback ina matokeo halisi — kama hati yako ya malipo imekubaliwa au imekataliwa.
...
Wakati `response_status_code` ni `"TIRA001"`, hati ya malipo imekubaliwa.
...
Wakati `response_status_code` ni kitu kingine tofauti na `"TIRA001"`, hati ya malipo imekataliwa. Angalia ukurasa wa [Misimbo ya Makosa](/sw/error-codes) kwa msimbo husika.
...
app.post("/tira/discharge-voucher-callback", async (req, res) => {
  ...
  if (result.extracted.response_status_code === "TIRA001") {
    ...
    console.error(
      `Hati ya malipo imekataliwa: ${result.extracted.response_status_code}`,
...
1. Kinapata jina sahihi la tag ya uthibitisho (`DischargeVoucherRes` → `DischargeVoucherResAck`)
...
| Sehemu               | Aina                               | Maelezo                                                                           |
| -------------------- | ---------------------------------- | --------------------------------------------------------------------------------- |
| `type`               | `"discharge_voucher"`              | Daima `"discharge_voucher"` kwa handler hii                                       |
| `extracted`          | `DischargeVoucherCallbackResponse` | Data iliyotolewa (tazama [Jibu la Callback](#jibu-la-callback-la-submit))         |
...
| -------------------- | --------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| Ya rasilimali maalum | `tira.dischargeVoucher.handleCallback(input)` | Ukiwa na endpoint tofauti kwa kila aina ya rasilimali                                          |
...
Programu kamili ya Express.js inayotuma hati ya malipo, kushughulikia callback, na kuthibitisha.
...
### Toleo Limekubaliwa vs Limekataliwa

Sehemu ya `offer_accepted` inaonyesha kama mdai amekubali au amekataa toleo la madai. Hii inaathiri mtiririko wa jumla wa hati ya malipo:

```js
// Mdai amekubali toleo
{
  offer_accepted: "Y",
  claim_offer_amount: 5000000,
  adjustment_amount: 0,
...
// Mdai hakukubali — marekebisho na upatanisho unatumika
{
  offer_accepted: "N",
  claim_offer_amount: 5000000,
  adjustment_amount: 1000000,
  adjustment_reason: "Mdai amepinga kiasi cha toleo la awali",
  reconciled_amount: 4000000,
  reconciliation_summary: "Makubaliano ya mwisho baada ya mazungumzo",
  // ...sehemu nyingine
}
```

Hata wakati `offer_accepted` ni `"Y"`, bado lazima utoe `adjustment_date`, `adjustment_reason`, `adjustment_amount`, `reconciliation_date`, `reconciliation_summary`, na `reconciled_amount` kwani zote ni sehemu zinazohitajika.
...
Kiasi chote cha fedha (`claim_offer_amount`, `adjustment_amount`, `reconciled_amount`) kinapaswa kuwa katika fedha iliyoainishwa. Kiwango cha ubadilishaji kinafomatiwa kwa desimali 2 kwenye XML.

### Wadai Waliorahisishwa

Tofauti na [Madai ya Awali](/sw/claim-intimation) ambayo inahitaji maelezo kamili ya wadai (jina, tarehe ya kuzaliwa, anwani, nambari ya simu, n.k.), hati ya malipo inatumia muundo rahisi wa wadai wenye sehemu 4 tu:
...
`"TIRA001"` kwenye jibu la utumaji inamaanisha "imepokelewa", si "imekubaliwa". Idhini au kukataliwa halisi kunakuja baadaye kupitia callback. Usiwaambie watumiaji wako hati ya malipo imekubaliwa wakati wa utumaji.
:::
...
`claim_reference_number` lazima iwe ile iliyorudishwa na TIRA kwenye callback ya taarifa ya madai, na `claim_assessment_number` lazima ilingane na tathmini iliyotumwa hapo awali. Kutumia nambari za kumbukumbu zisizo sahihi au zinazokosekana kutasababisha TIRA kukataa hati ya malipo.
:::
